...
func (s *WirelessService) Connect(ctx context.Context, address string) (string, error) {
	...
	if address == "" || !strings.Contains(address, ":") {
		return "", NewOperationError("connect_wireless", "wireless address is invalid", "address must use host:port format", false)
	}
	...
	if err != nil {
		return "", NewOperationError("connect_wireless", "failed to connect wireless device", err.Error(), true)
	}
	if result.ExitCode != 0 {
		return "", NewOperationError("connect_wireless", "wireless connect failed", strings.TrimSpace(result.Stderr), true)
	}
...
func (s *WirelessService) EnableTCPIP(ctx context.Context, serial string, port string) (string, error) {
	...
	if trimmedSerial == "" {
		return "", NewOperationError("enable_wireless_tcpip", "device serial is required", "serial must not be empty", false)
	}
	if trimmedPort == "" {
		trimmedPort = "5555"
	}
	...
	if err != nil {
		return "", NewOperationError("enable_wireless_tcpip", "failed to enable wireless TCP/IP mode", err.Error(), true)
	}
	if result.ExitCode != 0 {
		return "", NewOperationError("enable_wireless_tcpip", "tcpip command failed", strings.TrimSpace(result.Stderr), true)
	}
...
func (s *WirelessService) Disconnect(ctx context.Context, address string) (string, error) {
	...
	if err != nil {
		return "", NewOperationError("disconnect_wireless", "failed to disconnect wireless device", err.Error(), true)
	}
	if result.ExitCode != 0 {
		return "", NewOperationError("disconnect_wireless", "disconnect failed", strings.TrimSpace(result.Stderr), true)
	}
